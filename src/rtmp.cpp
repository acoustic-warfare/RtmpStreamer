...
/**
 * @brief Provides a command-line interface for controlling the RTMP and local streams.
 *
 * The `async_streamer_control_unit` function continuously reads commands from the standard input
 * and executes the corresponding streamer control functions based on the input.
 * 
 * Supported commands:
 ...
 * 
 * If an invalid command is entered, an error message is printed to the standard error.
 * The command loop will continue to process commands until the "quit" command is received.
 */
 ...
 *
 * The `cb_need_data` function is a static callback used by the GStreamer `appsrc` element.
 * It is called when the pipeline needs more data. The function sets the `want_data` flag
 * to `true` to indicate that the application should provide more data.
 * 
 * @param appsrc The GStreamer appsrc element requesting data.
 * @param size The size of the data needed.
 * @param user_data A pointer to user data; in this case, a boolean flag indicating the need for data.
 */
...
/**
 * @brief Callback function that resets the flag indicating that no more data is needed.
 *
 * The `cb_enough_data` function is a static callback used by the GStreamer `appsrc` element.
 * It is called when the pipeline has enough data. The function sets the `want_data` flag
 * to `false` to indicate that the application should stop providing data.
 * 
 ...
 *
 * The `RtmpStreamer` constructor initializes the streamer with default screen dimensions
 * and sets the `want_data` flag to `false`. It then calls `initialize_streamer` to set up
 * the GStreamer pipeline and elements.
 *
 * - Sets the screen width to 1024 pixels.
 * - Sets the screen height to 1024 pixels.
 * - Initializes the `want_data` flag to `false`.
 * - Calls the `initialize_streamer` function to configure the streaming pipeline.
 */
RtmpStreamer::RtmpStreamer() : screen_width(1024),
                               screen_height(1024),
                               want_data(false) {
    initialize_streamer();
}
 ...
 *
 * The `RtmpStreamer` constructor initializes the streamer with specified screen dimensions
 * and sets the `want_data` flag to `false`. It then calls `initialize_streamer` to set up
 * the GStreamer pipeline and elements.
 *
 ...
 * - Calls the `initialize_streamer` function to configure the streaming pipeline.
 */
RtmpStreamer::RtmpStreamer(uint width, uint height) : screen_width(width),
 ...
 *
 * The `~RtmpStreamer` destructor cleans up the GStreamer pipeline and releases resources.
 * It sets the pipeline state to `GST_STATE_NULL` to stop any ongoing streaming and unrefs
 * the pipeline object to free allocated memory.
 */
 ...
 *
 * The `start_stream` function initializes the `appsrc` element, sets up signal connections for
 * data handling, and transitions the pipeline to the `GST_STATE_PLAYING` state to start streaming.
 * 
 * Edge Cases:
 * - If the `appsrc` element cannot be retrieved from `source_bin`, an error message is printed
 ...
 *
 * The `stop_stream` function sets the state of the pipeline to `GST_STATE_NULL`, which stops
 * the streaming process and releases the resources used by the pipeline.
 */
...
/**
 * @brief Sends a frame to the GStreamer pipeline for streaming.
 *
 * The `send_frame` function processes a given OpenCV `cv::Mat` frame, converts it to the
 * required RGB format if necessary, and pushes it to the `appsrc` element of the GStreamer
 * pipeline.
 *
 * @param frame The `cv::Mat` frame to be sent.
 * @return `TRUE` if the frame was successfully sent, `FALSE` otherwise.
 *
 * Edge Cases:
 * - If the frame is empty, an error message is printed and `FALSE` is returned.
 * - If the `appsrc` does not currently require data, the function returns `FALSE`.
 * - If the frame is not in a supported format (not RGB or RGBA), an error message is printed and `FALSE` is returned.
 * - If there is an issue with the GStreamer clock or buffer duration, the function prints an error and exits.
 * - If pushing the buffer to `appsrc` fails, an error message is printed and `FALSE` is returned.
 */
 ...
 *
 * The `initialize_streamer` function configures the GStreamer pipeline with necessary bins and elements,
 * and links them to enable streaming. It creates and sets up:
 ...
 * 
 * Edge Cases:
 * - If any element creation or bin setup fails, an error message is printed and the program exits.
 * - If the `tee` element cannot be retrieved, an error message is printed and the program exits.
 * - If linking pads between elements fails, an error message is printed and the program exits.
 */
 ...
 *
 * The `set_element_state_to_parent_state` function retrieves the state of the parent element
 * and applies that state to the specified element. This ensures that the element's state aligns
 * with the parent element's state, facilitating consistent pipeline behavior.
 *
...
/**
 * @brief Starts the RTMP streaming by connecting the RTMP bin to the source bin.
 *
 * The `start_rtmp_stream` function checks if the RTMP bin is already connected to the pipeline.
 * If not, it connects the RTMP bin to the source bin using the `connect_sink_bin_to_source_bin` method.
 *
 * Edge Cases:
 * - If the RTMP bin is already connected, a message is printed and the function returns without making changes.
...
/**
 * @brief Stops the RTMP streaming by disconnecting the RTMP bin from the source bin.
 *
 * The `stop_rtmp_stream` function checks if the RTMP bin is connected to the pipeline.
 * If it is, the function disconnects the RTMP bin from the source bin and updates internal state.
 *
 * Edge Cases:
 * - If the RTMP bin is already disconnected, a message is printed and the function returns without making changes.
 * - If disconnecting the RTMP bin from the source bin fails, the program exits.
 * - The `src_rtmp_tee_pad` is set to `nullptr` after disconnection to avoid dangling pointers.
 */
...
/**
 * @brief Starts the local video streaming by connecting the local video bin to the source bin.
 *
 * The `start_local_stream` function checks if the local video bin is already connected to the pipeline.
 * If not, it connects the local video bin to the source bin using the `connect_sink_bin_to_source_bin` method.
 *
 * Edge Cases:
 * - If the local video bin is already connected, a message is printed and the function returns without making changes.
 * - If connecting the local video bin to the source bin fails, the program exits.
 */
...
/**
 * @brief Stops the local video streaming by disconnecting the local video bin from the source bin.
 *
 * The `stop_local_stream` function checks if the local video bin is currently connected to the pipeline.
 * If it is, the function disconnects the local video bin from the source bin and updates internal state.
 *
 * Edge Cases:
 * - If the local video bin is already disconnected, a message is printed and the function returns without making changes.
 * - If disconnecting the local video bin from the source bin fails, the program exits.
 * - The `src_local_tee_pad` is set to `nullptr` after disconnection to avoid dangling pointers.
 */
...
/**
 * @brief Disconnects a sink bin from a source bin and cleans up associated elements.
 *
 * The `disconnect_sink_bin_from_source_bin` function removes the connection between a sink bin and a source bin.
 * It handles unlinking and removing pads, releasing request pads from the `tee` element, and adjusting the pipeline state.
 *
 * @param source_bin The source bin element from which the sink bin will be disconnected.
 * @param sink_bin The sink bin element to be disconnected.
 * @param tee_pad The pad on the tee element used for the connection.
 * @param tee_ghost_pad_src_name The name of the ghost pad on the source bin that needs to be removed.
 * @return `true` if the disconnection was successful, `false` otherwise.
 *
 * Edge Cases:
 * - If any of the input parameters (bins, pads, or tee element) are invalid or NULL, an error message is printed and `false` is returned.
 * - If locking the pipeline state fails, an error message is printed and `false` is returned.
 * - If unlinking the pads or removing the ghost pad fails, an error message is printed and `false` is returned.
 * - If unlocking the pipeline state fails, an error message is printed and `false` is returned.
 */
...
/**
 * @brief Connects a sink bin to a source bin using a tee element and a ghost pad.
 *
 * The `connect_sink_bin_to_source_bin` function sets up the necessary connections to integrate a sink bin into a pipeline.
 * It involves adding the sink bin to the pipeline, requesting a pad from the tee element, and linking the ghost pad of the source bin to the sink bin.
 *
 * @param source_bin The source bin element from which the connection will be made.
 * @param sink_bin The sink bin element to be connected.
 * @param request_pad Pointer to a `GstPad` that will be set to the requested pad from the tee element.
 * @param tee_element_name The name of the tee element used to request a pad.
 * @param tee_ghost_pad_name The name of the ghost pad on the source bin that will be created.
 * @return `true` if the connection was successful, `false` otherwise.
 *
 * Edge Cases:
 * - If the source or sink bin is invalid, an error message is printed and `false` is returned.
 * - If locking the pipeline state fails, an error message is printed and `false` is returned.
 * - If adding the sink bin to the pipeline fails, an error message is printed and the program exits.
 * - If the tee element cannot be found, an error message is printed and `false` is returned.
 * - If linking the ghost pads fails, an error message is printed and `false` is returned.
 * - If unlocking the pipeline state fails, an error message is printed and `false` is returned.
 */
